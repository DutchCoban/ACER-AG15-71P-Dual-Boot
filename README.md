...
This guide uses a hybrid approach:
    ...
    Using the official Debian-signed bootloader to satisfy the motherboard's strict Shim/SBAT checks.
    Using a custom Machine Owner Key (MOK) to satisfy the bootloader's kernel checks, since Kali ships its kernels unsigned.
...
1. Download and Extract the Signed Bootloader Open your Kali terminal and run:
bash
...
2. Fix the GRUB Menu Paths Because the bootloader is from Debian, it looks for a configuration file in a debian folder. We must create this folder and set up an automated pointer so you don't get stuck at a grub> prompt.
bash
...
Because Kali Linux ships its kernels unsigned, any time you run apt upgrade and a new Linux kernel is installed, it will fail to boot (throwing a "you need to load the kernel first" error). Additionally, GRUB updates might overwrite our Debian-signed bootloader.
...
When to use it: Whenever you run sudo apt update && sudo apt upgrade and notice that a new linux-image or grub-efi package was installed, simply run:
bash
